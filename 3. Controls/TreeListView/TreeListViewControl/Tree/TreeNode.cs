...
namespace TreeListViewControl.Tree
{
    public sealed class TreeNode : INotifyPropertyChanged
    {
        ...
        // не понятно, зачем этот метод, если есть свойство set
        ...
        public TreeNode PreviousNode
        {
            get
            {
                if (_parent != null)
                {
                    int index = Index; // непонятно, зачем вводить тут переменнную
                    if (index > 0)
        ...
        // в конструктор TreeNode передаем три лист, необходимо для метода isExpanded и ChildrenChanged
